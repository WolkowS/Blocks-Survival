...
namespace CoreLib
{
    public static class Utils
    {
        ...
        // =======================================================================
        public static Rect GuiField(this Rect rect, int line)
        {
            return GuiField( rect, line, EditorGUIUtility.singleLineHeight);
        }
        
        public static Rect GuiField(this Rect rect, int line, float lineHeight)
        {
            return new Rect(rect.xMin, rect.yMin + lineHeight * line, rect.width, lineHeight);
        }
        ...
        public static Type GetFieldType(this SerializedProperty property)
        {
            ...
            for (var i = 0; i < slices.Length; i++)
                ...
                //gets info on field and its type
                    ...
                    type = _getFieldRecursively(slices[i]);

            return type;

            // -----------------------------------------------------------------------
            Type _getFieldRecursively(string path)
            {
                var t = type;

                while (t != null)
                {
                    var field = t.GetField(path, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.FlattenHierarchy | BindingFlags.Instance);
                    if (field != null)
