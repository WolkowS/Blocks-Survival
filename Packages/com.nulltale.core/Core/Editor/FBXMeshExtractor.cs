...
namespace CoreLib
{
    public class FBXMeshExtractor
    {
        ...
        private static bool ExtractMeshesMenuItemValidate()
        {
            ...
            {
                if (!AssetDatabase.GetAssetPath(UnityEditor.Selection.objects[i]).EndsWith(_sourceExtension))
                    return false;
            }
            return true;
        }
        ...
        private static void ExtractMeshesMenuItem()
        {
            ...
            for (int i = 0; i < UnityEditor.Selection.objects.Length; i++)
            {
                EditorUtility.DisplayProgressBar(_progressTitle, UnityEditor.Selection.objects[i].name, (float)i / (UnityEditor.Selection.objects.Length - 1));
                ExtractMeshes(UnityEditor.Selection.objects[i]);
            }
