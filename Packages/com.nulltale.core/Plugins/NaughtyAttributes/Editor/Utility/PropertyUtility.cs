...
namespace NaughtyAttributes.Editor
{
	public static class PropertyUtility
	{
		...
		public static bool IsEnabled(SerializedProperty property)
		{
			...
			{
				if (readOnlyAttribute.InPlaymode && Application.isPlaying)
					return false;
				
				if (readOnlyAttribute.InEditor && Application.isPlaying == false)
					return false;
			}
		...
		{
			var showIfPlaymodeAttribute = GetAttribute<ShowIfPlaymodeAttributeBase>(property);
			if (showIfPlaymodeAttribute != null)
			{
				if (showIfPlaymodeAttribute.IsPlaymode == false && Application.isPlaying)
					return false;
				
				if (showIfPlaymodeAttribute.IsPlaymode && Application.isPlaying == false)
					return false;
			}
