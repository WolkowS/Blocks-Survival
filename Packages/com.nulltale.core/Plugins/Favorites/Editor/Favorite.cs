...
namespace SmartFavorites
{
    public class Favorite : EditorWindow
    {
        ...
        private static void InitFavoriteSave()
        {
            if (!favoriteSave)
            {
                ...
                if (!favoriteSave)
                {
                    ...
                    AssetDatabase.CreateAsset(favoriteSave, "Assets/FavoriteSave.asset");
        ...
        public void OnGUI()
        {
            ...
            // Edit list name
            if (GUILayout.Button(EditorGUIUtility.IconContent("d_VerticalSplit"), toolbarIconButtonGuiStyle, GUILayout.ExpandWidth(false)))
            ...
            if (GUILayout.Button(EditorGUIUtility.IconContent("d_Toolbar Plus"), toolbarIconButtonGuiStyle, GUILayout.ExpandWidth(false)))
                ButtonAddFavoriteList();
            EditorGUI.BeginDisabledGroup(FavoriteSave.FavoriteLists.Count <= 1);
            if (GUILayout.Button(EditorGUIUtility.IconContent("d_Toolbar Minus"), toolbarIconButtonGuiStyle, GUILayout.ExpandWidth(false)))
